...
ResponseObject response = WebUI.callTestCase(findTestCase('Library/Community/Create Community'), [('communityName') : data.getValue(
            'CommunityName', 27), ('CommunityDesc') : data.getValue('CommunityDesc', 27), ('allowDiscussionBoards') : true
        , ('allowBlogPosts') : true, ('allowFileUploads') : true, ('communityCode') : 'code', ('isPrivate') : false, ('userName') : GlobalVariable.g_userName
        , ('userId') : GlobalVariable.g_UserID, ('role') : '', ('userStatus') : 'Joined', ('isHidden') : false, ('disableJoinRequest') : false], 
...
ResponseObject res = WebUI.callTestCase(findTestCase('Library/Community/Get members from community'), [('CommunityID') : CommunityID], FailureHandling.STOP_ON_FAILURE)
WSBuiltInKeywords.verifyResponseStatusCode(res, 200)

ResponseObject res2 = WebUI.callTestCase(findTestCase('Library/Community/Get Community details by ID'), [('communityID') : CommunityID],
	FailureHandling.STOP_ON_FAILURE)
 WS.verifyElementPropertyValue(res2, "userMembership.role", 3)
CustomKeywords.'validations.Assertions.VerifyElementValue'(CustomKeywords.'validations.Assertions.GetAttribute'(res2).userMembership.communityId, CommunityID)
 //String CommunityID123 = CustomKeywords.'getAttributeValue.GetAttributeValue.GetAttribute'(res2).id
//WS.verifyElementText(response, "userMembership", CommunityID)
 /*WS.verifyElementsCount(res2, "userMembership.userAvatar", 1);
String CommunityRole = CustomKeywords.'getAttributeValue.GetAttributeValue.GetAttribute'(res2).userMembership
