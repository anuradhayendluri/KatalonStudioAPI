...
ResponseObject response = WebUI.callTestCase(findTestCase('Library/Community/Create Community'), [('communityName') : data.getValue(
            'CommunityName', 46), ('CommunityDesc') : data.getValue('CommunityDesc', 46), ('allowDiscussionBoards') : true
...
WebUI.callTestCase(findTestCase('Library/Community/Add a member to community'), [('rating') : '3', ('userID') : GlobalVariable.ContributerID
        , ('userName') : GlobalVariable.ContributerUser, ('communityID') : CommunityID], FailureHandling.STOP_ON_FAILURE)

WebUI.callTestCase(findTestCase('Library/Community/Create discussion under community'), [('CommunityID') : CommunityID, ('DiscussionName') : data.getValue(
            'DiscussionName', 46), ('DiscussionDesc') : data.getValue('DiscussionDesc', 46)], FailureHandling.STOP_ON_FAILURE)
...
ResponseObject res= WebUI.callTestCase(findTestCase('Library/Contributer/Create a discussion thread'), [('DiscussionID') : finalString, ('Comment') : data.getValue(
            'Comment', 46)], FailureHandling.STOP_ON_FAILURE)

WSBuiltInKeywords.verifyResponseStatusCode(res, 200)

//WSBuiltInKeywords.containsString(response, data.getValue("Comment", 46), false, FailureHandling.STOP_ON_FAILURE)