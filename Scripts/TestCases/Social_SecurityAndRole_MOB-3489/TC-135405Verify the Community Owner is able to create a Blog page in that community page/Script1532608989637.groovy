...
/*ResponseObject response = WebUI.callTestCase(findTestCase('Library/Community/Create Community'), [('communityName') : data.getValue(
            'CommunityName', 32), ('CommunityDesc') : data.getValue('CommunityDesc', 32), ('allowDiscussionBoards') : true
...
ResponseObject res = WebUI.callTestCase(findTestCase('Library/Community/Create a blog under community'), [('blogName') : data.getValue(
            'BlogName', 32), ('blogDesc') : data.getValue('BlogDesc', 32), ('CommunityID') : GlobalVariable.CommunityID], FailureHandling.STOP_ON_FAILURE)

WSBuiltInKeywords.verifyResponseStatusCode(res, 200)

ResponseObject responseBlog= WebUI.callTestCase(findTestCase('Library/Community/Get Blogs'), [('CommunityID') : GlobalVariable.CommunityID], FailureHandling.STOP_ON_FAILURE)

WSBuiltInKeywords.containsString(responseBlog, data.getValue('BlogName', 32), false, FailureHandling.STOP_ON_FAILURE)

//WebUI.callTestCase(findTestCase('Library/Community/Delete Community'), [('communityID') : GlobalVariable.CommunityID], FailureHandling.STOP_ON_FAILURE)
