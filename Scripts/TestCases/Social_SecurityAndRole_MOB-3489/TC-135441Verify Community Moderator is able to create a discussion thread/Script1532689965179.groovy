...
ResponseObject response1 = WebUI.callTestCase(findTestCase('Library/Community/Add a member to community'), [('rating') : '2'
        , ('userID') : GlobalVariable.g_NameTemp, ('userName') : data.getValue('Moderator', 38), ('communityID') : GlobalVariable.CommunityID], FailureHandling.STOP_ON_FAILURE)

WSBuiltInKeywords.verifyResponseStatusCode(response1, 200)
// creating disscussion
ResponseObject res=WebUI.callTestCase(findTestCase('Library/Moderator/Create discussion under community'), [('CommunityID') : GlobalVariable.CommunityID, ('DiscussionName') : data.getValue("DiscussionName", 38)
	, ('DiscussionDesc') : data.getValue("DiscussionDesc", 38)], FailureHandling.STOP_ON_FAILURE)
// get discussion id
//String DiscussionID=CustomKeywords.'getAttributeValue.GetAttributeValue.GetAttribute'(res).discussions
ResponseObject res3= WebUI.callTestCase(findTestCase('Library/Community/Get Discussions'), [('CommunityID') : GlobalVariable.CommunityID], FailureHandling.STOP_ON_FAILURE)

String ResponseText=res3.getResponseText()
String id="\"id\""
...
String Discussionid = name.substring(0, lastIndex).replace("\"", "").replaceAll("[:]","").trim()
// creating disscussion thread
ResponseObject res2= WebUI.callTestCase(findTestCase('Library/Moderator/Create a discussion thread'), [('DiscussionID') : Discussionid, ('Comment') : data.getValue("Comment", 38)], 
    FailureHandling.STOP_ON_FAILURE)

WSBuiltInKeywords.verifyResponseStatusCode(res2, 200)
